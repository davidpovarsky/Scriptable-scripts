...
/* שורת חיפוש */
#search-container { position:fixed; top:-100px; left:50%; transform:translateX(-50%); width:90%; max-width:500px; z-index:200; transition:top 0.3s ease; padding:10px; background:var(--bg); border-radius:20px; box-shadow:0 4px 20px rgba(0,0,0,0.5); }
#search-container.visible { top:70px; }
#search-input { width:100%; padding:12px 20px; border-radius:25px; border:2px solid #374151; background:var(--card); color:var(--text); font-size:16px; outline:none; transition:border 0.2s; }
#search-input:focus { border-color:var(--accent); }
#search-results { max-height:400px; overflow-y:auto; margin-top:10px; background:var(--card); border-radius:15px; display:none; }
#search-results.visible { display:block; }
.search-result-item { padding:12px 16px; border-bottom:1px solid #374151; cursor:pointer; transition:background 0.2s; }
.search-result-item:last-child { border-bottom:none; }
.search-result-item:hover { background:rgba(255,255,255,0.05); }
.search-result-name { font-size:16px; font-weight:600; margin-bottom:4px; }
.search-result-code { font-size:13px; color:#9ca3af; margin-bottom:4px; }
.search-result-desc { font-size:12px; color:#6b7280; }
#search-overlay { position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.5); z-index:150; display:none; }
...
`;
  ...
  if (IS_SCRIPTABLE) {
    // Scriptable - CSS ו-JS מוטמעים (inline)
    return `
<!DOCTYPE html>
