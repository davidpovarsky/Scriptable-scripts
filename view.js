...
<head>
  ...
  <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@20..48,100..700,0..1,-50..200&icon_names=directions_bus" />
    ...
    allPayloads.forEach(p => {
      ...
      stops.forEach(s => {
        if (typeof s.lat === "number" && typeof s.lon === "number") {
          ...
          L.circleMarker(ll, { radius: 3, weight: 1 }).bindTooltip((s.stopName||"")+(s.stopCode?" ("+s.stopCode+")":""),{direction:"top",offset:[0,-4]}).addTo(group);
          allLatLngs.push(ll);
        }
      });
      // Vehicles
