...
<head>
  ...
  .bus-icon-wrapper svg { display: block; }
  </style>
</head>
...
<script>
  ...
  function ensureMapInstance(allPayloads) {
    ...
    mapRouteLayers.forEach(l => { try { mapInstance.removeLayer(l); } catch (e) {} });
    mapRouteLayers = [];
    ...
    allPayloads.forEach(p => {
      ...
      stops.forEach(s => {
        if (typeof s.lat === "number" && typeof s.lon === "number") {
          ...
          L.circleMarker(ll, { radius: 3, weight: 1 })
        ...
        const bubbleSvg = `
          <svg width="32" height="20" xmlns="http://www.w3.org/2000/svg">
            ...
            <text x="16" y="14"
                  ...
                  fill="black"
                  text-anchor="middle">${routeNumber}</text>
          </svg>
        `;
  ...
  function renderAll() {
    ...
    payloads.forEach((payload) => {
      ...
      const {
        header, routeNumSpan, headsignSpan,
      ...
      stops.forEach((stop, idx) => {
        ...
        timeline.innerHTML =
          ...
          '<div class="timeline-circle" style="border-color:'+opColor+'"></div>' +
        ...
        main.innerHTML =
          '<div class="stop-name"><span class="seq-num" style="color:'+opColor+'">'+(idx+1)+'.</span><span>'+stop.stopName+'</span></div>' +
      ...
      setTimeout(() => {
        stopsList.querySelectorAll(".bus-icon-wrapper").forEach(e => e.remove());
