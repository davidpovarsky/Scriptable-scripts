...
module.exports.getHtml = function() {
  ...
  if (isScriptable) {
    try {
      ...
      // ===== JS =====
      // חשוב: busModelLayer לפני mapManager (כדי שהמחלקות יהיו קיימות בזמן init)
      const jsFiles = [
        'modules/ui/utils.js',
...
<head>
  ...
  <!-- ✅ Three.js + GLTFLoader (כמו בדוגמה שלך) -->
  ...
  <style>
    ...
    .error-steps ol { margin: 0; padding-right: 20px; }
    .error-steps li { margin: 10px 0; color: #fff; }
    .error-code {
      background: #000;
      padding: 10px;
      border-radius: 4px;
    ...
    .error-link { color: #4af; text-decoration: none; font-weight: bold; }
  </style>
...
<body class="mode-map-only">
  ...
  <script>
    ...
    // ✅ בדיקת Three/Loader (אם זה false — הבעיה ברשת/חסימה)
    console.log("🧪 THREE exists?", typeof THREE !== "undefined");
    console.log("🧪 GLTFLoader exists?", (typeof THREE !== "undefined") && (typeof THREE.GLTFLoader !== "undefined"));
